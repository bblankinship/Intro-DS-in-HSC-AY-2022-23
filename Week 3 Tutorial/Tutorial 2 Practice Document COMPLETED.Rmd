---
title: "Tutorial 2: Completed Practice Document"
...
# your code here: 
## install packages code is commented out as you only need to run it once on your computer 
...
Now add a pipe with the `kable()` function and knit the file, comparing the two outputs. Which do you prefer? 
...
Explore some of the `kableExtra` styling options and further customize the table output

```{r task-5}
gap_data %>% 
  select(-pop) %>% 
  sample_n(10) %>% 
  kable(caption = "Sample data from the Gapminder dataset", digits = 2) %>%
...
The gapminder dataset has variables with the following names: `r gapminder %>% names()`
